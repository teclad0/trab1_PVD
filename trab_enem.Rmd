...
set.seed(1)
enem.df_subconjunto <- enem.df %>% slice_sample(n= 5000) %>% select(where(is.character), -NU_ANO, -TX_RESPOSTAS_CN, -TX_RESPOSTAS_CH, -TX_RESPOSTAS_LC, -TX_RESPOSTAS_MT)
  ...
  p_value= numeric()
   ...
   for (col1 in colnames(dados)) {
    for(col2 in colnames(dados)){
      ...
      print(col2)
      #li_med <- medidas(dados[,col1], dados[,col2])
      p <-  chisq.test(unlist(dados[,col1]), unlist(dados[,col2]), simulate.p.value = TRUE)$p.value
      #Retornar apenas atributos com valores relevantes
      #if(!is.na(li_med$correlacao) & !is.na(li_med$determinacao)){
      #if(li_med$correlacao > 0.7 | li_med$correlacao < -0.7 | li_med$determinacao > 0.7){
...
rst_chisq <- rst_chisq[!duplicated(apply(rst_chisq,1,function(x) paste(sort(x),collapse=''))),]
rst_chisq
